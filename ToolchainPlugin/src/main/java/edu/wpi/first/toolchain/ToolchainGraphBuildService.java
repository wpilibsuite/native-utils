...
public abstract class ToolchainGraphBuildService
              ...
              for (GccExtension tcExt : missingToolChains) {
                ...
                boolean installing =
                    graph.getAllTasks().stream()
                        .anyMatch(
                            t ->
                                t instanceof InstallToolchainTask
                                    && ((InstallToolchainTask) t)
                                        .getDescriptorName()
                                        .equals(descriptor.getName()));
                if (!installing) {
                  ETLogger logger =
                      ETLoggerFactory.INSTANCE.create(this.getClass().getSimpleName());
                  TreeFormatter formatter = new TreeFormatter();
                  descriptor.explain(formatter);
                  logger.info(formatter.toString());

                  boolean optional =
                      descriptor.getOptional().get()
                          || tcExt
                              .getProject()
                              .hasProperty("toolchain-optional-" + descriptor.getName());
                  ;
                  if (optional) {
                    if (!singlePrintPerPlatform
                        || !skippedPlatforms.contains(descriptor.getName())) {
                      skippedPlatforms.add(descriptor.getName());
                      logger.logStyle(
                          "Skipping builds for "
                              + descriptor.getName()
                              + " (toolchain is marked optional)",
                          StyledTextOutput.Style.Description);
                    }
                  } else if (tcExt.isUsed()) {
                    logger.logError("=============================");
                    logger.logErrorHead("No Toolchain Found for " + descriptor.getName());
                    logger.logErrorHead(
                        "Run `./gradlew " + descriptor.getInstallTaskName() + "` to install one!");
                    logger.logErrorHead("");
                    logger.logErrorHead(
                        "You can ignore this error with -Ptoolchain-optional-"
                            + descriptor.getName());

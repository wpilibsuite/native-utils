...
public class ToolchainDescriptor implements ToolchainDescriptorBase {
  ...
  public ToolchainDescriptor(
      Project project, String name, String gccName, Property<Boolean> optional) {
    this.name = name;
  ...
  public ToolchainDiscoverer discover() {
    return discoverers.stream()
        .flatMap(x -> x.getDiscovererList().stream())
        .filter(ToolchainDiscoverer::valid)
        .findFirst()
        .orElse(null);
  }
  ...
  public AbstractToolchainInstaller getInstaller() {
    return installers.stream()
        .filter(AbstractToolchainInstaller::installable)
        .findFirst()
        .orElse(null);
  }

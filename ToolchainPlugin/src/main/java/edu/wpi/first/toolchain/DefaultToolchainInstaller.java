...
public class DefaultToolchainInstaller extends AbstractToolchainInstaller {
  ...
  public DefaultToolchainInstaller(
      OperatingSystem os, Provider<URL> source, Provider<File> installDir, String subdir) {
    this.os = os;
  ...
  public void install(Project project) {
    ...
    if (extractDir.exists()) project.delete(extractDir.getAbsolutePath());
    extractDir.mkdirs();
    ...
    project.copy(
        (CopySpec c) -> {
          ...
          else if (dst.getName().endsWith(".zip")) tree = project.zipTree(dst);
          else throw new GradleException("Don't know how to extract file type: " + dst.getName());
    ...
    if (installDir.exists()) project.delete(installDir.getAbsolutePath());
    installDir.mkdirs();

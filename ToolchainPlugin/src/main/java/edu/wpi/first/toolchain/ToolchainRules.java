...
public class ToolchainRules extends RuleSource {
  ...
  void addDefaultToolchains(
      NativeToolChainRegistryInternal toolChainRegistry, ExtensionContainer extContainer) {

    final ToolchainExtension ext = extContainer.getByType(ToolchainExtension.class);

    ext.getToolchainDescriptors()
        .all(
            desc -> {
                          ...
                          if (discoverer != null) {
                            tc.eachPlatform(
                                toolchain -> {
                                  toolchain
                                      .getcCompiler()
                                      .setExecutable(discoverer.toolName("gcc"));
                                  toolchain
                                      .getCppCompiler()
                                      .setExecutable(discoverer.toolName("g++"));
                                  ...
                                  toolchain
                                      ...
                                      .setExecutable(discoverer.toolName("ar"));
                                });
  ...
  void disableCrossTests(BinaryContainer binaries) {
    ...
    Action<RunTestExecutable> eachTaskAction =
        new Action<RunTestExecutable>() {

          Spec<Task> forceTaskToNotRunSpec =
              new Spec<Task>() {
                @Override
    ...
    for (GoogleTestTestSuiteBinarySpec binary :
        binaries.withType(GoogleTestTestSuiteBinarySpec.class)) {
  ...
  void addDefaultPlatforms(
      final ExtensionContainer extContainer, final PlatformContainer platforms) {
    final ToolchainExtension ext = extContainer.getByType(ToolchainExtension.class);
    ...
    if (ext.registerPlatforms) {
      ...
      for (CrossCompilerConfiguration config : ext.getCrossCompilers()) {
        ...
        NativePlatform configedPlatform =
            platforms.maybeCreate(config.getName(), NativePlatform.class);
  ...
  @Validate
  void checkEnabledToolchains(
      final BinaryContainer binaries,
      final NativeToolChainRegistry toolChains,
  ...
  public static OrderedStripTask configureOrderedStrip(
      AbstractLinkTask link, GccExtension gcc, NativeBinarySpec binary) {
    ...
    if (strip == null) {
      ...
      strip =
          linkExt
              .getExtensions()
              .create("orderedStrip", OrderedStripTask.class, tcExt, binary, link, gcc, project);
      link.doLast(strip);
    }
    return strip;
  }
  ...
  @BinaryTasks
  void createNativeStripTasks(
      final ModelMap<Task> tasks, NativeBinarySpec binary, final ExtensionContainer extContainer) {
    final ToolchainExtension ext = extContainer.getByType(ToolchainExtension.class);

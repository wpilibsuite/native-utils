...
public abstract class ToolchainDiscoverer implements Named {
  ...
  public static ToolchainDiscoverer createDiscoverer(
      String name,
      ToolchainDescriptorBase descriptor,
      Provider<File> rootDir,
      Function<String, String> composer,
      Project project) {
    return project
        .getObjects()
        .newInstance(ToolchainDiscoverer.class, name, descriptor, rootDir, composer);
  }

  public static ToolchainDiscovererProperty createProperty(
      String name,
      ToolchainDescriptorBase descriptor,
      Provider<File> rootDir,
      Function<String, String> composer,
      Project project) {
    ToolchainDiscovererProperty prop =
        project.getObjects().newInstance(ToolchainDiscovererProperty.class, name);
    Provider<ToolchainDiscoverer> disc =
        project.provider(
            () ->
                project
                    .getObjects()
  ...
  private Optional<GccMetadata> metadata(File file, DiagnosticsVisitor visitor) {
    ...
    SearchResult<GccMetadata> searchresult =
        metadataProvider.getCompilerMetaData(
            new ArrayList<File>(),
            compilerSpec -> {
              compilerSpec.executable(file);
            });
    if (visitor != null) searchresult.explain(visitor);
    return Optional.ofNullable(searchresult.getComponent());
  }

  public static List<File> systemPath(
      Project project, ToolchainGraphBuildService tce, Function<String, String> composer) {
  ...
  public static ToolchainDiscovererProperty forSystemPath(
      Project project,
      ToolchainGraphBuildService tce,
      ToolchainDescriptorBase descriptor,
      Function<String, String> composer) {
    ToolchainDiscovererProperty prop =
        project.getObjects().newInstance(ToolchainDiscovererProperty.class, "PathList");

    Provider<List<ToolchainDiscoverer>> p =
        project.provider(
            () -> {
              ...
              for (File f : systemPath(project, tce, composer)) {
                ...
                disc.add(
                    ToolchainDiscoverer.createDiscoverer(
                        "Path" + (i++), descriptor, fp, composer, project));
              }
              return disc;
            });

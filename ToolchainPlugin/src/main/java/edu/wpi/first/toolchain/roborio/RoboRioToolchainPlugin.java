...
public class RoboRioToolchainPlugin implements Plugin<Project> {
  ...
  public void apply(Project project) {
    ...
    opensdk =
        new OpenSdkToolchainBase(
            baseToolchainName,
            roborioExt,
            project,
            RoboRioToolchainExtension.INSTALL_SUBDIR,
            "roborio-academic",
            prefixProvider,
            toolchainExt.getToolchainGraphService());

    CrossCompilerConfiguration configuration =
        project.getObjects().newInstance(CrossCompilerConfiguration.class, NativePlatforms.roborio);
    ...
    ToolchainDescriptor descriptor =
        new ToolchainDescriptor(
            project, toolchainName, toolchainName + "Gcc", configuration.getOptional());
    ...
    descriptor
        ...
        .add(
            ToolchainDiscoverer.createProperty(
                "FRCHome", descriptor, fp, opensdk::composeTool, project));

...
public class ToolchainPlugin implements Plugin<Project> {
  ...
  public void apply(Project project) {
    Provider<ToolchainGraphBuildService> serviceProvider =
        project
            .getGradle()
            .getSharedServices()
            .registerIfAbsent("toolchainGraph", ToolchainGraphBuildService.class, spec -> {});
    serviceProvider.get().configure(project.getGradle());

    ext =
        project
            .getExtensions()
            .create("toolchainsPlugin", ToolchainExtension.class, project, serviceProvider.get());
    ...
    ext.getToolchainDescriptors()
        .all(
            (ToolchainDescriptorBase desc) -> {
              if (serviceProvider.get().registerInstallTask(desc.getInstallTaskName())) {
                project
                    .getTasks()
                    .register(
                        desc.getInstallTaskName(),
                        InstallToolchainTask.class,
                        (InstallToolchainTask t) -> {
                          t.setGroup("Toolchains");
                          t.setDescription(
                              "Install Toolchain for "
                                  + desc.getName()
                                  + " if installers are available.");
    ...
    project
        ...
        .whenReady(
            (TaskExecutionGraph graph) -> {
              // Sort into buckets based on the descriptor, then cancel all but the first entry in
              // each
              // of those sublists, ensuring only one instance of each install task may run.
              List<InstallToolchainTask> installTasks =

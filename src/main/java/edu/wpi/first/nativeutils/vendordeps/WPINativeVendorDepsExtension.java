...
public class WPINativeVendorDepsExtension {
  ...
  private void initializeJsonDep(
      String uuid,
      String jsonName,
      List<CppArtifact> cppDeps,
      String prefix,
      ExtensiblePolymorphicDomainObjectContainer<NativeDependency> dependencyContainer) {

    String depName = prefix + "_" + uuid + "_" + jsonName;

    AllPlatformsCombinedNativeDependency combinedDep =
        dependencyContainer.create(depName, AllPlatformsCombinedNativeDependency.class);
    ...
    for (CppArtifact cpp : cppDeps) {
      ...
      WPIVendorMavenDependency vendorDep =
          dependencyContainer.create(name, WPIVendorMavenDependency.class);
  ...
  public void initializeNativeDependencies() {
    ...
    dependencyContainer.registerFactory(
        WPIVendorMavenDependency.class,
        name -> {

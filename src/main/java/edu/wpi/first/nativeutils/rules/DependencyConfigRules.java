...
public class DependencyConfigRules extends RuleSource {
  ...
  // @ComponentType
  // public void registerNativeBinarySpecExtension(TypeBuilder<NativeBinarySpec>
  // builder) {
  ...
  //   for (NativeBinarySpec oBinary : binaries.withType(NativeBinarySpec.class)) {

  //     if (!oBinary.getTargetPlatform().getOperatingSystem().isWindows()) {
  //       continue;
  //     }
  ...
  // super("Cannot find delegated dependency: " + name + " for binary: " +
  // binary);
  ...
  // public static class MissingDependencyVariantException extends
  // RuntimeException {
  ...
  // public MissingDependencyVariantException(String name, NativeBinarySpec
  // binary, NativeUtilsExtension.NamedNativeDependencyList namedDep) {
  // super("Cannot find delegated dependency with proper variant: " + name + " for
  // binary: " + binary);
  // this.dependencyName = name;
  // this.binary = binary;
  ...
  // public UnknownDependencyTypeException(String name, NativeBinarySpec binary,
  // BaseLibraryDependencySet set) {
  ...
  // private void addDependency(String depName, NativeBinarySpec binary,
  // FrcNativeBinaryExtension binaryExt,
  // NamedDomainObjectSet<NativeUtilsExtension.NamedNativeDependencyList> sets,
  // boolean allowOptional, List<String> systemLibs) {
  // // See if dependency exists in sets
  // NativeUtilsExtension.NamedNativeDependencyList depSet =
  // sets.findByName(depName);
  // if (depSet == null) {
  ...
  // CombinedLibraryDependencySet combined =
  // (CombinedLibraryDependencySet)baseSet;
  ...
  // @Validate
  // public void configureFrcDependencies(BinaryContainer binaries,
  // ExtensionContainer extensions) {
  // NativeUtilsExtension nue = extensions.getByType(NativeUtilsExtension.class);
  // NamedDomainObjectSet<NativeUtilsExtension.NamedNativeDependencyList> sets =
  // nue.getNativeLibraryDependencySets();
  ...
  // @BinaryTasks
  // public void addLinkerArgs(ModelMap<Task> tasks, final NativeBinarySpec
  // binary) {
  ...
  // NativeUtilsExtension extension =
  // extensions.getByType(NativeUtilsExtension.class);

  // NamedDomainObjectCollection<DependencyConfig> dependencies =
  // extension.getDependencyConfigs();
  // NamedDomainObjectCollection<CombinedDependencyConfig> combinedDependencies =
  // extension
  ...
  // List<String> sharedMatchers = Arrays.asList("**/*.so", "**/*.so.*",
  // "**/*.dylib", "**/*.lib");
  // List<String> runtimeMatchers = Arrays.asList("**/*.so", "**/*.so.*",
  // "**/*.dylib", "**/*.dll");
  // List<String> sharedExcludes = Arrays.asList("**/*.so.debug",
  // "**/*.so.*.debug");
  // List<String> runtimeExcludes = Arrays.asList("**/*.so.debug",
  // "**/*.so.*.debug");
  ...
  // String mavenBase = dependency.getGroupId() + ":" + dependency.getArtifactId()

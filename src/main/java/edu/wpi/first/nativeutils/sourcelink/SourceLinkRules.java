...
public class SourceLinkRules extends RuleSource {
  ...
  @BinaryTasks
  public void createSourceLinkSharedBinaryTasks(
            ...
            link -> {
              handleLinkedComponent(
                  ...
                  binary
                      ...
                      .getTasks()
                      .named(
                          SourceLinkPlugin.SOURCE_LINK_ROOT_TASK_NAME,
                          SourceLinkGenerationTask.class),
                  binary.getBuildTask().getProject());
            });
  }

  @BinaryTasks
  public void createSourceLinkExecutableTasks(
      ModelMap<Task> tasks, NativeExecutableBinarySpecInternal binary) {
    if (!binary.getTargetPlatform().getOperatingSystem().isWindows()) {
      return;
    }
    binary
  ...
  private void handleLinkedComponent(
      NativeBinarySpecInternal binary,
      AbstractLinkTask linkTask,
      TaskProvider<SourceLinkGenerationTask> rootGenTask,
      Project project) {
    String sourceLinkName = binary.getNamingScheme().getTaskName("generateSourceLinkFile");

    TaskProvider<LinkerSourceLinkGenerationTask> sourceGenTask =
        project.getTasks().register(sourceLinkName, LinkerSourceLinkGenerationTask.class);

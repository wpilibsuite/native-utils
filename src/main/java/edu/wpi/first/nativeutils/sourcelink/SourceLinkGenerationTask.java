...
public class SourceLinkGenerationTask extends DefaultTask {
  ...
  private void resolveSubmodules(Repository repo) throws IOException, GitAPIException {
    ...
    String regexPattern =
        "^(?<url>(?<companyurl>(?:https:\\/\\/)?github\\.com\\/(?<company>[^\\/]+))\\/(?<project>[^\\/]+?)(\\.git)?)$";
    ...
    String remoteBase =
        "https://raw.github.com/" + company + "/" + project + "/" + currentHead + "/*";
    ...
    try (Git git = Git.wrap(repo)) {
      ...
      for (Entry<String, SubmoduleStatus> sm : submodules.entrySet()) {
        try (Repository subRepo =
            SubmoduleWalk.getSubmoduleRepository(repo, sm.getValue().getPath())) {
          if (subRepo == null) {
            getLogger().warn("Invalid submodule found at subpath " + sm.getValue().getPath());
            getLogger()
                .warn(
                    "Likely caused by submodules not initialized. SourceLink will not work correctly");
            continue;
          }
  ...
  public void execute() throws IOException {
    ...
    jsonList.add(json);
    Files.write(
        sourceLinkFile.get().getAsFile().toPath(),
        jsonList,

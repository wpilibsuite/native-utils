...
public class ExportsConfigRules extends RuleSource {
    ...
    @BinaryTasks
    public void createExportsSharedBinaryTasks(ModelMap<Task> tasks, SharedLibraryBinarySpecInternal binary) {
        ...
        binary.getTasks().withType(AbstractLinkTask.class, link -> {
            ...
            TaskProvider<ExportsGenerationTask> exportsTask = project.getTasks().register(exportsName,
                    ExportsGenerationTask.class, task -> {
                        task.setArchitecture(binary.getTargetPlatform().getArchitecture().getName());
                        ...
                        link.getLinkerArgs().add(task.getDefFile().map(x -> "/DEF:" + x.getAsFile().toString()));
